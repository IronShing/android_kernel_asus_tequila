...
static bool is_profile_load_required(struct fg_gen4_chip *chip)
{
	...
	if (val & PROFILE_LOAD_BIT) {
		...
		if (profiles_same) {
			...
			pr_warn("Battery profile is same, not loading it\n");
		...
		pr_info("Profiles are different, loading the correct one\n");
	} else {
...
void long_full_cap_monitor_polling(int time) {
	cancel_delayed_work(&g_fg->long_full_cap_monitor_work);
	schedule_delayed_work(&g_fg->long_full_cap_monitor_work, time * HZ);
}

extern bool usbin_full_flag;
void long_full_cap_monitor_worker(struct work_struct *work)
{
	...
	rc = fg_get_msoc_raw(g_fg, &msoc_raw);
	if (rc < 0) {
	...
	if(bat_cap < 100 && usbin_full_flag == true) {
		usbin_full_flag = false;
	}

	rc = power_supply_get_property(g_fg->usb_psy, POWER_SUPPLY_PROP_PD_ACTIVE, &prop);
	BAT_DBG("%s bat_cap=%d, bat_cur=%d, msoc_raw=%d, isINOV=%d, PD_active=%d, asus_vid=0x%02x, Fake(%d,%d)\n", __func__,bat_cap,bat_cur, msoc_raw, is_INOV_trigger, prop.intval, asus_vid, b_Is_full_pa, b_Is_99_pa);

	fg_get_online_status(g_fg);
	if(g_fg->online_status && (bat_cur >= 0) && !is_INOV_trigger && prop.intval != 0 && asus_vid == USB_VID_ASUS){
		if(bat_cap >= 99){
			if(bat_cur <=FV_CURRENT){
	...
	if (g_fg->fg_psy && (b_Is_99_pa || b_Is_full_pa)) {
		//Need to force updating SOC to user space again.
		power_supply_changed(g_fg->fg_psy);
	}
...
extern char *asus_id[];
extern void asus_dump_reg(void);
static int print_battery_status(void) {
		...
		snprintf(battInfo, sizeof(battInfo), "report Capacity ==>%d, FCC:%dmAh, BMS:%d, V:%dmV, Cur:%dmA, ",
			...
			bat_vol/1000,
			bat_cur/1000);
		snprintf(battInfo, sizeof(battInfo), "%sTemp:%d.%dC, BATID:%d, CHG_Status:%d(%s), CHG_Mode:%s, , APSD_Result:%s, UFP_Mode:%s, BAT_HEALTH:%s, smart:%d, ASUS_ADAPTER_ID(%s) \n",
			battInfo,
			bat_temp/10,
			bat_temp%10,
			g_fg->batt_id_ohms,
...
void static update_gauge_status_worker(struct work_struct *dat)
{
		...
		if (!g_fg->profile_load_status) {
			...
			if (ret == 0) {
				...
				BAT_DBG_E("charger not ready yet, delay 5s\n");
				qpnp_smbcharger_polling_data_worker(5);
			}
		}
}
...
static int fg_gen4_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&g_fg->long_full_cap_monitor_work, long_full_cap_monitor_worker); //ASUS_BSP LiJen implement the asus owns algorithm of detection full capacity
	//ASUS_BSP ---
	...
	qpnp_smbcharger_polling_data_worker(10);//ASUS_BSP Start the status report of fuel gauge
...
//ASUS_BSP +++
	...
	asus_extcon_set_name(fg->bat_ver_extcon, "C11P1904-O-03-0000-29.01.0.82");
